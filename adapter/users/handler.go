...
func (a *UsersHandler) GetUserData(c fiber.Ctx) error {
	...
	return utils.ResponseJSON(c, fiber.StatusOK, "User data retrieved successfully", "", fiber.Map{
		"id":           user.ID,
		"username":     user.Username,    // ชื่อ player → ใช้ทุก scene
		"email":        user.Email,
		"character":    user.Character,
		"chatbot_name": user.ChatbotName, // ชื่อ chatbot → "" จนกว่าจะถึงจุดในเนื้อเรื่อง
	})
}

func (a *UsersHandler) SetChatbotName(c fiber.Ctx) error {
	var input UsersModels.SetChatbotNameInput
	if err := c.Bind().Body(&input); err != nil {
		return utils.ResponseJSON(c, fiber.StatusBadRequest, "Invalid request body", err.Error(), nil)
	}
	userID := c.Locals("userID")
 
	message, err := a.UsersUsecase.SetChatbotName(uint(userID.(float64)), &input)
	if err != nil {
		return utils.ResponseJSON(c, fiber.StatusInternalServerError, message, err.Error(), nil)
	}
	return utils.ResponseJSON(c, fiber.StatusOK, message, "", fiber.Map{
